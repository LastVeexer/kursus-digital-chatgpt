<template>
  <div class="scrollable-content">
    <h2 class="mb-4">Tambah Kursus Baru</h2>
    <!-- Form untuk menambah kursus baru -->
    <form @submit.prevent="createKursus">
...
export default {
  ...
  methods: {
    // Method untuk membuat kursus baru
    createKursus() {
      ...
      // Mengirim data kursus ke server untuk ditambahkan
      api
        .post('/kursus', this.kursus)
        .then(() => {
          alert('Kursus berhasil ditambahkan!')
          this.$router.push('/kursus') // Navigasi kembali ke halaman daftar kursus setelah berhasil
        })
        .catch((error) => {
          console.error('Terjadi kesalahan saat menambahkan kursus:', error)
        })
    },
